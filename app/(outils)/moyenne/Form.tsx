...
import { format, fraction, mean } from "mathjs";
import { useState } from "react";

type MoyenneFormProps = {};

export const MoyenneForm = (props: MoyenneFormProps) => {
  ...
  const onSubmit = () => {
    let moyenne = mean(values);

    const hasDecimal = !Number.isInteger(moyenne);

    if (hasDecimal) {
      const fractionPrint = fraction(moyenne);
      const formattedFraction = format(fractionPrint, { fraction: "ratio" });
      moyenne = formattedFraction as any;
    }
    setResult(moyenne.toString());
  };
