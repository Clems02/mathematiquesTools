...
import { getPGCD } from "@/src/lib/outils/pgcd";
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} style={{ maxWidth: "600px" }}>
      <Stack spacing={2} alignItems={"center"}>
        ...
        <Button type="submit">Obtenir le PGCD</Button>
      </Stack>
      {result && (
        <Stack pt={4}>
          <Typography textAlign={"center"} variant="h6">
            Le PGCD de {formatNumberPrintSpace(result.number1)} et{" "}
            {formatNumberPrintSpace(result.number2)} est{" "}
            {formatNumberPrintSpace(result.pgcd)}.
          </Typography>
        </Stack>
      )}
    </form>
